...
// The scroll carousel below cycles through every project, but only some have
// a real glTF yet — the rest borrow Capsule C1's model as a placeholder so
// the navigation itself (speed, crossfade, name change) can be judged before
// any more assets exist. Swap entries in here per project as real ones are made.
const PROJECT_MODEL_URLS: Record<string, string> = {
  'mr-takahashi': '/models/mr-takahashi.glb'
}

// Fish Man is a hand-animated PNG sequence (Unity sprite export, 12fps),
// not a glTF — CapsuleStage flips through it on a billboard instead of
// loading a model when `spriteFrames` is set.
const FISH_MAN_FRAMES = Array.from(
  { length: 14 },
  (_, i) => `/sprites/fish-man-idle/Fish_Man_Idle_${String(i).padStart(5, '0')}.png`
)
const PROJECT_SPRITES: Record<string, { frames: string[]; fps: number }> = {
  'slider-engine': { frames: FISH_MAN_FRAMES, fps: 12 }
}

// Mecha Station has no modelled asset, real or placeholder — CapsuleStage
// hand-builds the register/reader/monitor counter and loops this footage
// on the monitor instead of loading a glTF.
const PROJECT_VIDEOS: Record<string, string> = {
  'mecha-station': '/videos/mecha-station-hero.mp4'
}

// The rest of the hand-built products — a game case, a plain card, a video
// in a frame, an animated block stack — go through CapsuleStage's generic
// `renderProduct` slot rather than adding a new named prop per shape.
const PROJECT_PRODUCTS: Record<string, (scale: number) => ReactNode> = {
  // No cover art yet — DiscHolder renders an inset placeholder panel until
  // `coverUrl` is passed in.
  'grand-theft-auto-v': (scale) => <DiscHolder scale={scale} />,
  'red-dead-redemption-2': (scale) => <DiscHolder scale={scale} />,
  'wyte-card': (scale) => <WyteCard scale={scale} />,
  stitchfam: (scale) => <VideoFrame videoUrl="/videos/stitchfam-hero.mp4" scale={scale} />,
  'block-builder': (scale) => <BlockBuilder scale={scale} />
}

const CAROUSEL_PROJECTS = workProjects.map((project) => {
  const sprite = PROJECT_SPRITES[project.id]
  const video = PROJECT_VIDEOS[project.id]
  const product = PROJECT_PRODUCTS[project.id]
  return {
    id: project.id,
    title: project.title,
    modelUrl: sprite || video || product !== undefined ? undefined : (PROJECT_MODEL_URLS[project.id] ?? MODEL_URL),
    spriteFrames: sprite?.frames,
    spriteFps: sprite?.fps,
    videoUrl: video,
        ...
        <div className="ch-stage">
          ...
          {opened ? (
            <CapsuleStage
              ...
              spriteFrames={CAROUSEL_PROJECTS[carouselIndex].spriteFrames}
              spriteFps={CAROUSEL_PROJECTS[carouselIndex].spriteFps}
              videoUrl={CAROUSEL_PROJECTS[carouselIndex].videoUrl}
              renderProduct={CAROUSEL_PROJECTS[carouselIndex].renderProduct}
              spinRef={spinRpmRef}
              {...productProps}
          ...
          {opened && carouselStarted ? (
            <CapsuleStage
              ...
              spriteFrames={CAROUSEL_PROJECTS[nextCarouselIndex].spriteFrames}
              spriteFps={CAROUSEL_PROJECTS[nextCarouselIndex].spriteFps}
              videoUrl={CAROUSEL_PROJECTS[nextCarouselIndex].videoUrl}
              renderProduct={CAROUSEL_PROJECTS[nextCarouselIndex].renderProduct}
              spinRef={spinRpmRef}
              {...productProps}
              introFrom={intro.productRise}
              introTurn={intro.productTurn}
